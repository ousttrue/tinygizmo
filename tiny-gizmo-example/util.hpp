...
struct camera
{
    ...
    ray get_ray_from_pixel(const linalg::aliases::float2 &pixel, const rect &viewport)
    {
        const float x = 2 * (pixel.x - viewport.x0) / viewport.width() - 1, y = 1 - 2 * (pixel.y - viewport.y0) / viewport.height();
        const linalg::aliases::float4x4 inv_view_proj = inverse(this->get_viewproj_matrix(viewport.aspect_ratio()));
        ...
        return {this->position, p1.xyz() * p0.w - p0.xyz() * p1.w};
    }
};
